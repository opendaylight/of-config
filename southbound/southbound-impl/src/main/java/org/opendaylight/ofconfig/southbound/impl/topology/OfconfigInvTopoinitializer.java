...
public class OfconfigInvTopoinitializer {
    ...
    public void initializeOfconfigTopology(DataBroker db, TopologyId topologyId,
            LogicalDatastoreType type) {
        InstanceIdentifier<Topology> path = InstanceIdentifier.create(NetworkTopology.class)
                .child(Topology.class, new TopologyKey(topologyId));
        ReadWriteTransaction transaction = db.newReadWriteTransaction();
        initializeTopology(transaction, type);
        CheckedFuture<Optional<Topology>, ReadFailedException> ofconfigTp =
                transaction.read(type, path);
        try {
    ...
    private void initializeTopology(ReadWriteTransaction transaction, LogicalDatastoreType type) {
        ...
        CheckedFuture<Optional<NetworkTopology>, ReadFailedException> topology =
                transaction.read(type, path);
        try {
